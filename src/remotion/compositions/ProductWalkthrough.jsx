...
function ChapterLabel({ label }) {
  ...
  const opacity = interpolate(frame, [0, 15, 30, 45], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
      ...
      <div style={{ display: "flex", gap: 16, height: "100%" }}>
        ...
        <div style={{ width: 180, background: "#fff", borderRadius: 8, padding: 16, border: "1px solid #e5e7eb" }}>
          ...
          {["Inbox (12)", "Priority", "Campaigns", "Prospects", "Calendar"].map((item, i) => {
            const itemOpacity = interpolate(frame, [10 + i * 6, 20 + i * 6], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            return (
...
function ComposerScene() {
  const frame = useCurrentFrame();
  const typingProgress = interpolate(frame, [30, 120], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ background: "#f9fafb", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
      <ChapterLabel label="Email Composer" />
      <div style={{ maxWidth: 700, margin: "40px auto 0", background: "#fff", borderRadius: 12, padding: 28, border: "1px solid #e5e7eb" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 20, opacity: interpolate(frame, [130, 145], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
          <div style={{ background: "#111", color: "#fff", padding: "8px 20px", borderRadius: 4, fontSize: 13, fontWeight: 600 }}>Send</div>
          <div style={{ background: "#FFEFEB", color: "#FF5C39", padding: "8px 16px", borderRadius: 4, fontSize: 13, fontWeight: 600 }}>Templates</div>
...
function CampaignScene() {
  const frame = useCurrentFrame();
  const progressWidth = interpolate(frame, [40, 120], [0, 100], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  return (
    <AbsoluteFill style={{ background: "#f9fafb", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
      <ChapterLabel label="Campaign Launch" />
      <div style={{ maxWidth: 800, margin: "20px auto 0" }}>
        <div style={{ fontSize: 22, fontWeight: 700, color: "#111", marginBottom: 20, opacity: interpolate(frame, [0, 15], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
...
function SDRScene() {
  ...
  return (
    <AbsoluteFill style={{ background: "#f9fafb", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 800, margin: "20px auto 0" }}>
        <div style={{ fontSize: 22, fontWeight: 700, color: "#111", marginBottom: 4, opacity: interpolate(frame, [0, 15], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
          AI Prospect Discovery
        </div>
        <div style={{ fontSize: 14, color: "#6b7280", marginBottom: 20, opacity: interpolate(frame, [10, 25], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
          3 new prospects found matching "Enterprise IT, 200+ employees"
        </div>
        {prospects.map((p, i) => {
          const cardOpacity = interpolate(frame, [30 + i * 20, 50 + i * 20], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
          return (
  ...
  return (
    <AbsoluteFill style={{ background: "#f9fafb", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 800, margin: "20px auto 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", opacity: interpolate(frame, [0, 20], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", gap: 12, marginBottom: 12, opacity: interpolate(frame, [20 + i * 12, 35 + i * 12], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
        ...
        <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", opacity: interpolate(frame, [30, 50], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
    ...
    <AbsoluteFill style={{ background: "#f9fafb", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 800, margin: "20px auto 0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
          {stats.map((stat, i) => {
            const animatedValue = interpolate(frame, [10 + i * 8, 60 + i * 8], [0, stat.value], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            return (
        ...
        <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb" }}>
          ...
          <div style={{ display: "flex", alignItems: "end", gap: 8, height: 120 }}>
            {[35, 42, 38, 47, 52, 49, 55, 58, 53, 61, 64, 59].map((val, i) => {
              const barHeight = interpolate(frame, [30 + i * 5, 50 + i * 5], [0, val * 1.8], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
              return (
...
function RevealScene() {
  ...
  return (
    <AbsoluteFill style={{ background: "#020617", opacity, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
      <div style={{ fontSize: 18, color: "rgba(255,255,255,0.4)", fontFamily: "Inter, system-ui, sans-serif", marginBottom: 16, opacity: interpolate(frame, [20, 40], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
        All running on
      </div>
      <div style={{ display: "flex", gap: 32, marginBottom: 40 }}>
        {["Your Storage", "Your Domain", "Your Data"].map((item, i) => {
          const itemOpacity = interpolate(frame, [40 + i * 15, 60 + i * 15], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
          return (
